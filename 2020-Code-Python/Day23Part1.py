...
# after cup 1?
# -----------------------------------------------------------------------------------
# This doesn't output the full answer, but it shows the position of the cups.
# So I would list the cups clockwise from wherever 1 is.
# -----------------------------------------------------------------------------------

import copy

data_given = "167248359"
cup_array = list(data_given)

i = 0
while i < len(cup_array):
  cup_array[i] = int(cup_array[i])
  i = i + 1
...
i = 1
while i < len(cup_array):
  if lowest > cup_array[i]:
    lowest = copy.deepcopy(cup_array[i])
  if highest < cup_array[i]:
    highest = copy.deepcopy(cup_array[i])
  i = i + 1

i = 0
while i < 100:
  curr_cup = cup_array[0]
  cup1 = copy.deepcopy(cup_array[1])
  cup2 = copy.deepcopy(cup_array[2])
  cup3 = copy.deepcopy(cup_array[3])
  cup_array.remove(cup1)
  cup_array.remove(cup2)
  cup_array.remove(cup3)
  des_cup = curr_cup - 1
  while des_cup not in cup_array:
    if des_cup == curr_cup:
      des_cup = des_cup - 1
    elif des_cup < lowest:
      des_cup = highest
      ...
      des_cup = des_cup - 1
  cup_array.insert(cup_array.index(des_cup) + 1, copy.deepcopy(cup3))
  cup_array.insert(cup_array.index(des_cup) + 1, copy.deepcopy(cup2))
  cup_array.insert(cup_array.index(des_cup) + 1, copy.deepcopy(cup1))

  cup_array.append(copy.deepcopy(cup_array[0]))
  cup_array.pop(0)
