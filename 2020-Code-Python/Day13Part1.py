...
my_time = int(input_array[0])
bus_array = re.split(",", input_array[1])
...
while i < len(bus_array):
  test_time = 0
  while my_time > test_time:
    test_time = test_time + int(bus_array[i])
  best_time.append(test_time)
  i = i + 1
...
earliest = 0
j = 0
while j < len(best_time):
  if j == 0:
    earliest = 0
  elif best_time[earliest] > best_time[j]:
    earliest = best_time.index(best_time[j])
  j = j + 1
print(earliest)
print(bus_array[earliest])
print(best_time[earliest])
print(my_time)

print(int(bus_array[earliest]) * (best_time[earliest] - my_time))
