...
players_array = content.split("\n\n")

player1 = players_array[0].split("\n")
player2 = players_array[1].split("\n")
...
def recursive_combat(deck1, deck2):
  ...
  while (len(deck1) != 0) and (len(deck2) != 0):
    if [deck1, deck2] in copyboth:
      return "player1", deck1
    else:
      copyboth.append([copy.deepcopy(deck1), copy.deepcopy(deck2)])
    if (deck1[0] <= (len(deck1) - 1)) and (deck2[0] <= (len(deck2) - 1)):
      winner, longest = recursive_combat(deck1[1:deck1[0] + 1], deck2[1:deck2[0] + 1])
      if winner == "player1":
        deck1.append(copy.deepcopy(deck1[0]))
        deck1.append(copy.deepcopy(deck2[0]))
      else:
        deck2.append(copy.deepcopy(deck2[0]))
        deck2.append(copy.deepcopy(deck1[0]))
...
count = 0
windeck = []
winplayer, windeck = recursive_combat(player1, player2)
print(windeck)

while j < len(windeck):
  print(windeck[j] * (len(windeck) - j))
  count = count + (windeck[j] * (len(windeck) - j))
  j = j + 1
