...
while j < len(input_array):
  count_array.append(int(input_array[j]))
  j = j + 1
...
while i < 2020:
  last_spoken = spoken
  if last_spoken in count_array:
    k = 0
    count = 0
    while k < len(count_array):
      if count_array[k] == last_spoken:
        count = count + 1
      k = k + 1
    if count < 2:
      spoken = 0
      ...
      while find_two < 2:
        if count_array[m] == last_spoken:
          ...
          find_two = find_two + 1
        m = m - 1
      spoken = last_inst - second_inst

  else:
    spoken = 0
  count_array.append(spoken)
  i = i + 1

print(count_array[len(count_array) - 1])
