...
# -----------------------------------------------------------------------------------
# (sorry, seems like I printed all the bags as a check and never removed it)
# -----------------------------------------------------------------------------------
...
while h < len(valid_bags):
    i = 0
    curr_check = valid_bags[h][:-1]
    while i < len(rules_array):
        current_rule = re.split(' contain \d+ | contain |, \d+|\.',
                                rules_array[i])
        current_rule.remove("")
        j = 1
        while j < len(current_rule):
            if ((current_rule[j].endswith(curr_check)) or
                (current_rule[j][:-1].endswith(curr_check))) and (
                    current_rule[0] not in valid_bags):
                valid_bags.append(current_rule[0])
                j = len(current_rule)
                print("found one")
                print(valid_bags)
            j = j + 1
